...
class MessageContent(BaseModel):
    ...
    file_url: str | None = Field(
        default=None,
        description="由 backend-data 对象存储接口返回的统一资源 URL",
    )
    file_name: str | None = Field(default=None, description="文件原始名称（可选）")
    card_json: str | dict | None = Field(
        default=None, description="交互卡片 JSON 结构或对象（平台特定备用）"
    )
    card_data: QuestionCardData | dict | None = Field(
        default=None, description="解耦的全局公共卡片数据模型"
    )
...
class StandardMessage(BaseModel):
    ...
    message_id: str | None = Field(
        default=None, description="原始消息 ID（回复时关联，发新消息时为空）"
    )
