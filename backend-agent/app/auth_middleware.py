...
def _auth_error(request: Request, message: str, *, status_code: int = 401) -> JSONResponse:
...
def _client_ip(request: Request) -> str | None:
    ...
    优先取反向代理转发的前置 IP（X-Forwarded-For 首段），
    回退到 uvicorn 记录的直连 client.host。
    """
...
def _guest_route_allowed(request: Request, user: dict[str, Any]) -> bool:
    role = str(user.get("role") or "").strip().lower()
    if role != "guest":
        return True
    method = request.method.upper()
    path = request.url.path
    allowed = role_allowed_for_route(
        request.app.state.database_path,
        role_key="guest",
        method=method,
        path=path,
    )
    permission_key = route_permission_for_path(
        request.app.state.database_path,
        method=method,
        path=path,
    )
...
def install_auth_middleware(app: FastAPI) -> None:
    ...
    async def _guard_api_session(request: Request, call_next):
        ...
        config = get_yaml_config(request.app.state.project_root).as_dict()
        ...
        try:
            ...
            return _auth_error(request, exc.message, status_code=exc.status_code)
        ...
        if not _external_ip_allowed(config, client_ip):
            return _auth_error(request, "来源 IP 不在外部访问白名单内", status_code=403)

        token = extract_bearer_token(request.headers.get("Authorization"))
        if not token:
            return _auth_error(request, "请先登录")

        # ── 双 Token (Redis) ──
        dual_mgr = get_dual_token_manager()
        if dual_mgr is None:
            return _auth_error(request, "认证服务未就绪", status_code=503)

        try:
            token_user = await validate_access_token(token)
        except DualTokenError as exc:
            return _auth_error(request, exc.msg, status_code=exc.status_code)

        # 组装 user dict 以兼容现有 request.state 契约
        stored_user = None
        if token_user.role != "guest":
            stored_user = get_console_user(request.app.state.database_path, token_user.username)
            if stored_user is None:
                return _auth_error(request, "账号不存在，请重新登录", status_code=401)
        user = {
        ...
        if not user["is_active"]:
            return _auth_error(request, "账号已停用，请联系管理员", status_code=403)
        if not _guest_route_allowed(request, user):
            return _auth_error(request, "游客账号无访问权限", status_code=403)
