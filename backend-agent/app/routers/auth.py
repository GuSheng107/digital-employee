...
def _auth_service_error(exc: Exception) -> AppError:
    return AppError("认证服务暂时不可用", status_code=503)
...
async def login(
    ...
    try:
        ...
        raise _auth_service_error(exc) from exc
    return {
        "user": user,
...
async def reset_password(
    ...
    try:
        await revoke_all_user_tokens(username)
    except DualTokenError as exc:
        raise AppError(exc.msg, status_code=exc.status_code) from exc
    except Exception as exc:
        raise _auth_service_error(exc) from exc
